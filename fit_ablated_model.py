...
def get_args():
    ...
    parser.add_argument('--stan_model', type=str, required=True, help="Name of ablated stan model to fit on data")
...
def main():
    ...
    i_for_factor_analysis = set(i_with_t0).intersection(i_with_allvalues)
    x_t0 = [simulated_data['observed_data']['X'][i] for i in i_for_factor_analysis]

    init_values = initialize_factor_analysis_from_data(x_t0, K_constrained=args.k_constrained, n_chains=args.num_chains, min_constrained_value_size=args.min_constrained_value_size, noise_in_initialization=0.05)
